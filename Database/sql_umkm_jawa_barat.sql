...
CREATE TABLE skala_umkm (
    id_skala INT(11) AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE kabupaten_kota (
    id_kabupaten_kota INT(11) AUTO_INCREMENT PRIMARY KEY,
...
-- Tabel umkm
CREATE TABLE umkm (
    id_umkm INT(11) AUTO_INCREMENT PRIMARY KEY,
...
-- View 1: view_umkm_detail
CREATE VIEW view_umkm_detail AS SELECT a.nama_usaha, b.nama_lengkap AS nama_pemilik, c.nama_kategori, d.nama_skala, e.nama_kabupaten_kota, a.tahun_berdiri FROM umkm a JOIN pemilik_umkm b ON a.id_pemilik = b.id_pemilik JOIN kategori_umkm c ON a.id_kategori = c.id_kategori JOIN skala_umkm d ON a.id_skala = d.id_skala JOIN kabupaten_kota e ON a.id_kabupaten_kota = e.id_kabupaten_kota ORDER BY DESC  ;

-- View 2: view_pemilik_dan_usaha
CREATE VIEW view_pemilik_dan_usaha AS SELECT a.nik, a.nama_lengkap, a.jenis_kelamin, a.nomor_telepon, a.email, b.nama_usaha FROM pemilik_umkm a JOIN umkm b ON a.id_pemilik = b.id_pemilik;

-- View 3: view_produk_umkm
CREATE VIEW view_produk_umkm AS SELECT b.nama_usaha, a.nama_produk, a.deskripsi_produk, a.harga FROM produk_umkm a JOIN umkm b ON a.id_umkm = b.id_umkm;

-- View 4: view_umkm_menengah
CREATE VIEW view_umkm_menengah AS SELECT a.nama_usaha, b.nama_lengkap AS nama_pemilik, a.total_aset, a.omzet_per_tahun FROM umkm a JOIN pemilik_umkm b ON a.id_pemilik = b.id_pemilik JOIN skala_umkm c ON a.id_skala = c.id_skala WHERE c.nama_skala = 'Menengah';

-- View 5: view_umkm_per_kota
CREATE VIEW view_umkm_per_kota AS SELECT a.nama_kabupaten_kota, COUNT(b.id_umkm) AS jumlah_umkm FROM kabupaten_kota a LEFT JOIN umkm b ON a.id_kabupaten_kota = b.id_kabupaten_kota GROUP BY a.nama_kabupaten_kota;

SELECT * FROM view_umkm_detail ORDER BY tahun_berdiri ASC;
...
-- View 1: Gabungan dari 2 tabel (orders dan customers)
CREATE VIEW view_customer_orders AS SELECT a.order_id, a.order_date, a.total_price, a.status, b.name AS customer_name, b.email, b.phone FROM orders a JOIN customers b ON a.customer_id = b.customer_id;

-- View 2: Gabungan dari 3 tabel (order_items, orders, products)
CREATE VIEW view_order_details AS SELECT a.order_item_id, b.order_id, c.name AS product_name, a.banyak_item, a.price, b.order_date FROM order_items a JOIN orders b ON a.order_id = b.order_id JOIN products c ON a.id_produk = c.id_produk;

-- View 3: Gabungan dari 2 tabel dengan kondisi (orders dan customers - hanya yang Completed)
CREATE VIEW view_completed_orders AS SELECT a.order_id, a.order_date, a.total_price, b.name AS customer_name, b.email FROM orders a JOIN customers b ON a.customer_id = b.customer_id WHERE a.status = 'Completed';

-- View 4: Fungsi agregasi dari 2 tabel (customers dan orders)
CREATE VIEW view_customer_total_spending AS SELECT b.customer_id, b.name AS customer_name, COUNT(a.order_id) AS total_orders, SUM(a.total_price) AS total_spent FROM orders a JOIN customers b ON a.customer_id = b.customer_id GROUP BY b.customer_id, b.name;

-- View 5: View kustom yang berguna (produk dengan stok rendah)
CREATE VIEW view_low_stock_products AS SELECT a.id_produk, a.name, a.kategori, a.stok, a.harga FROM products a WHERE a.stok < 10;

CREATE VIEW view_most_spend AS SELECT MAX(a.total_price) AS total_spend, b.name FROM orders a JOIN customers b ON a.customer_id = b.customer_id;
SELECT * FROM view_customer_orders;
SELECT * FROM view_order_details;
SELECT * FROM view_completed_orders;
SELECT * FROM view_customer_total_spending;
SELECT * FROM view_low_stock_products;
SELECT * FROM view_most_spend;
